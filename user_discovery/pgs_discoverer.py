    ...
    with conn.cursor() as cur:
        ...
        if len(records) > 0:
            ...
            for index in range(len(split_fls)):
                fld_data_list.append("{}: {}, ".format(split_fls[index], records[0][index]))
...
def print_user_info(p_config, pf):
    ...
    with psycopg2 \
        ...
        try:
            ...
            records += print_grant_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
            records += print_permissions_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
        except Exception as e:
            print("Can not continue: {}".format(e))
            return 0
    ...
    with psycopg2 \
            ...
            as conn:
        records += print_events(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += print_accessed_identities_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += print_identity_login_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += print_identity_tasks_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += print_session_tokens_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
    ...
    with psycopg2 \
            .connect("host={} port={} dbname={} user={} password={}"
                             .format(config_li.get("host"), config_li.get("port"),consumable_data_db_name, config_li.get("user"),
                                     config_li.get("password"))) \
            as conn:
        records += print_access_key_usage(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += print_auto_pilot_recommendations_0(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += print_identity_access_advisor_tasks(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += print_identity_entitlements_usage_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += print_identity_risk_score(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += print_identity_task_usage(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += print_privilege_escalation(conn, data_source_id, shard_id, ref_id, pf+pf)
    return records


####=================---------------------- Clean UP -------------------=================

def delete_identity_info(conn, data_source_id, shard_id, user_name, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identities_{} WHERE data_source_id={} and identity_type=1 and identity_name='{}'"\
                                 .format(shard_id, data_source_id, user_name)
        cur.execute(statement)
        if cur.rowcount > 0:
            print("{}\tDeleted number of records from identities table for user name id {} : {}"
                  .format(pf, user_name, cur.rowcount))
            return cur.rowcount
    with conn.cursor() as cur:
        statement = "DELETE FROM identities_transient_{} WHERE data_source_id={} and identity_type=1 and identity_name='{}'"\
                                 .format(shard_id, data_source_id, user_name)
        cur.execute(statement)
        if cur.rowcount > 0:
            print("{}\tDeleted number of records from transient_identities table for user name id {} : {}"
                  .format(pf, user_name, cur.rowcount))
            return cur.rowcount
        else:
            raise Exception("No identity {} found.".format(user_name))
...
def delete_grant_summary(conn, data_source_id, shard_id, ref_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_grants_summary_{} WHERE data_source_id={} and identity_ref_id={}"\
                   .format(shard_id, data_source_id, ref_id)
        cur.execute(statement)
        print("{}\tDeleted number of records for user ref id {} : {}".format(pf, ref_id, cur.rowcount))
        return cur.rowcount


def delete_permissions_summary(conn, data_source_id, shard_id, ref_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_permissions_{} WHERE data_source_id={} and identity_ref_id={}"\
                                 .format(shard_id, data_source_id, ref_id)
        cur.execute(statement)
        print("{}\tDeleted number of records for user ref id {} : {}".format(pf, ref_id, cur.rowcount))
        return cur.rowcount
...
def delete_events(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM events_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of events performed by identity: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_accessed_identities_summary(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identities_accessed_{} WHERE data_source_id={} and (source_identity_id='{}' or target_identity_id='{}')"\
                                 .format(shard_id, data_source_id, ident_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of accesses with identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_login_summary(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_logins_{} WHERE data_source_id={} and identity_id='{}'"\
                                 .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of logins by identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_tasks_summary(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_tasks_{} WHERE data_source_id={} and identity_id='{}'"\
                                 .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of tasks by identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_session_tokens_summary(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM session_tokens_{} WHERE data_source_id={} and identity_id='{}'"\
                                 .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of session tokens by identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_access_key_usage(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM accesskey_entitlements_usage_summary_{} WHERE data_source_id={} and owner_id='{}'"\
                                 .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of Access Key Usages for identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_auto_pilot_recommendations_0(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM auto_pilot_recommendations_{} WHERE data_source_id={} and identity_id='{}'"\
                                 .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of autopilot recommendations with identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_access_advisor_tasks(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_access_advisor_tasks_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of services with advisor tasks with identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_entitlements_usage_summary(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_entitlements_usage_summary_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of Entitlements Usages for identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_risk_score(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_risk_score_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of risk scores categories with identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_identity_task_usage(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM identity_task_usage_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of task usages for identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount


def delete_privilege_escalation(conn, data_source_id, shard_id, ident_id, pf):
    with conn.cursor() as cur:
        statement = "DELETE FROM privilege_escalation_{} WHERE data_source_id={} and identity_id='{}'" \
            .format(shard_id, data_source_id, ident_id)
        cur.execute(statement)
        print("{}Deleted number of privilege escalations for identity id: {}".format(pf, cur.rowcount))
        return cur.rowcount
...
def delete_info(p_config, pf):
    ...
    with psycopg2 \
        ...
        p_config["user_ref_id"] = ref_id
        p_config["user_ident_id"] = ident_id
        records += delete_grant_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_permissions_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
        delete_identity_info(conn, data_source_id, shard_id, p_config.get("user_name"), pf + pf)
    ...
    with psycopg2 \
            .connect("host={} port={} dbname={} user={} password={}"
                             .format(config_hi.get("host"),  config_hi.get("port"), activities_db_name, config_hi.get("user"),
                                     config_hi.get("password"))) \
            as conn:
        records += delete_events(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += delete_accessed_identities_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += delete_identity_login_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += delete_identity_tasks_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += delete_session_tokens_summary(conn, data_source_id, shard_id, ident_id, pf+pf)
    ...
    with psycopg2 \
            .connect("host={} port={} dbname={} user={} password={}"
                             .format(config_li.get("host"), config_li.get("port"),consumable_data_db_name, config_li.get("user"),
                                     config_li.get("password"))) \
            as conn:
        records += delete_access_key_usage(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_auto_pilot_recommendations_0(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_identity_access_advisor_tasks(conn, data_source_id, shard_id, ident_id, pf+pf)
        records += delete_identity_entitlements_usage_summary(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_identity_risk_score(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_identity_task_usage(conn, data_source_id, shard_id, ref_id, pf+pf)
        records += delete_privilege_escalation(conn, data_source_id, shard_id, ref_id, pf+pf)
    return records